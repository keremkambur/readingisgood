...
namespace ReadingIsGood.DataLayer.Extensions
{
    public static class TypeExtensions
    {
        ...
        {
            if (_typeToFriendlyName.TryGetValue(type, out var friendlyName)) return friendlyName;
            ...
            if (type.IsGenericType)
            {
                ...
                if (backtick > 0) friendlyName = friendlyName.Remove(backtick);
        ...
        {
            return type.Namespace?.Split(new[] {"."}, StringSplitOptions.RemoveEmptyEntries).LastOrDefault() ??
                   string.Empty;
        }
    }
}